...
class UserAccountTest(TestCase):
    ...
    def test_utility_photo(self) -> None:
        company_obj:Company = UserAccount.objects.filter(id=self.comp_id).first().entrepreneuraccount \
            .company_set.first()
        self.assertIsNotNone(company_obj)

        photos = []
        for i in range(1, 1+randint(5, 12)):
            temp = CompanyPhoto(company=company_obj, img=mock_image_field(), img_index=i)
            temp.full_clean()
            temp.save()
            photos.append(temp)

        for i in range(5):
            temp:Company = Company.objects.get(id=company_obj.id)
            photos = [photo for photo in temp.companyphoto_set.all().order_by('id')]
            randomized_set = list(range(-999, 999))
            for photo in photos:
                photo.img_index = choice(randomized_set)
                randomized_set.remove(photo.img_index)
                photo.full_clean()
                photo.save()
            photos_sorted = sorted(photos, key=lambda x: x.img_index)
            photos_sorted = [img.id for img in photos_sorted]

            recalculate_img_index(company_obj)
            temp:Company = Company.objects.get(id=company_obj.id)
            photos_result = [photo.id for photo in temp.companyphoto_set.all().order_by('img_index')]
            self.assertEqual(photos_sorted, photos_result)
